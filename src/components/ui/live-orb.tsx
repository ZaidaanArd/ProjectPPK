...
void main() {
  ...
  if (u_luminous > 0.5) {
    if (u_mode > 0.5) {
      ...
      float warp = fbm(p + vec2(t, -t));
      float marble = sin(p.x * 4.0 + p.y * 3.0 + warp * 15.0 + t);
      body = mix(u_c1, u_c2, smoothstep(-0.65, 0.65, marble));
      body = mix(body, u_c3, 0.25 * fbm(p * 4.0 + warp));
    }
    float rim = pow(1.0 - n.z, 3.0);
    float light = pow(max(dot(n, normalize(vec3(-0.45, 0.65, 1.0))), 0.0), 18.0);
    body = mix(body, vec3(1.0, 0.94, 0.98), clamp(rim * 0.8 + light * 0.65, 0.0, 0.9));
  }
...
export function createLiveOrb(
  ...
  const uLuminous = gl.getUniformLocation(program, "u_luminous")
  ...
  const tick = (now: number) => {
    ...
    gl.uniform1f(uLuminous, options.appearance === "luminous" ? 1 : 0)
...
export function LiveOrb({
  ...
  }, [variant, appearance, color, eyeColor, colors, interactive, blink])
  ...
  return (
    ...
    >
      ...
      {appearance === "luminous" ? (
        <div
          aria-hidden="true"
          className="pointer-events-none absolute inset-[3%] rounded-full border border-white/65 shadow-[inset_0_0_18px_rgba(255,255,255,0.35)]"
        />
      ) : null}
    </div>
  )
